                                        ...
                                        @foreach(\App\Currency\Currency::all() as $currency)
                                            <tr>
                                                ...
                                                <th data-highlight style="text-align: right">
                                                    <div>
                                                        <span data-c>{{ number_format(\Illuminate\Support\Facades\DB::table('games')->where('demo', '!=', true)->where('user', $user->_id)->where('currency', $currency->id())->sum('wager'), 8, '.', '') }}</span>
                                                        <i class="{{ $currency->icon() }}" style="color: {{ $currency->style() }}"></i>
                                                    </div>
                                                </th>
                                                <th data-highlight>
                                                    <div>
                                                        <span data-c>{{ number_format(\App\Investment::getUserProfit($currency, \Carbon\Carbon::minValue(), $user), 8, '.', '') }}</span>
                                                        <i class="{{ $currency->icon() }}" style="color: {{ $currency->style() }}"></i>
                                                    </div>
                                                </th>
                                            </tr>
                            ...
                            <div class="settingsNotify" style="text-align: left">
                                ...
                                @if(!(auth()->user()->tfa_enabled ?? false))
                                    ...
                                    <div class="mt-2">
                                        ...
                                        <canvas id="qrcanvas" data-text="{{ $tfa->getQRText('Bitlucy.com', $secret) }}" class="d-flex ml-auto mr-auto">
                                    </div>
