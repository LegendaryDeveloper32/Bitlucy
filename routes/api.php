...
Route::middleware('auth')->prefix('promocode')->group(function() {
    Route::post('activate', function() {
        ...
        auth()->user()->balance(Currency::find($promocode->currency))->add($promocode->sum, \App\Transaction::builder()->message('Promocode')->get());
        return success();
    });
    ...
    Route::post('bonus', function() {
        ...
        if(auth()->user()->balance(auth()->user()->clientCurrency())->get() > 0) return reject(2, 'Balance is greater than zero');
